...
def RunSimulationsOnMdp(_mdp, simulation_steps, agents_to_run, runs_for_specific_mdp, _method_type_list):
    # creating simulation
    simulators = {method: SimulatorFactory(method, _mdp, agents_to_run) for method in _method_type_list}
    simulator_inputs = {method: SimInputFactory(method, simulation_steps, agents_to_run) for method in _method_type_list}

    chain_activation = {key: 0 for key in _method_type_list}
    reward_eval = {key: 0 for key in _method_type_list}

    for i in range(runs_for_specific_mdp):

        for method in _method_type_list:
...
if __name__ == '__main__':
    ...
    for i in range(mdp_num):
        ...
        activations, reward_eval = RunSimulationsOnMdp(mdp,
                                                       ...
                                                       _method_type_list=method_type_list)
